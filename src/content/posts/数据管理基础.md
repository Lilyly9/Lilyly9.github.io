...
- [U6 关系数据理论](#u6-关系数据理论)
  ...
  - [Armstrong公理系统](#armstrong公理系统)
    - [1. Armstrong公理系统-基本规则](#1-armstrong公理系统-基本规则)
    - [2. Armstrong公理系统 -扩充规则](#2-armstrong公理系统--扩充规则)
    - [3. 核心基础概念](#3-核心基础概念)
      - [3.1 函数依赖的逻辑蕴涵](#31-函数依赖的逻辑蕴涵)
      - [3.2 函数依赖集的闭包 F^+（定义 6.12）](#32-函数依赖集的闭包-f定义-612)
      - [3.3 属性集的闭包 $X\_F^+$（定义 6.13）](#33-属性集的闭包-x_f定义-613)
      - [函数依赖集的覆盖与等价](#函数依赖集的覆盖与等价)
    - [2. Armstrong 公理系统](#2-armstrong-公理系统)
      - [2.1 三条基本规则（公理）](#21-三条基本规则公理)
      - [2.2 扩充规则（由基本规则推导而来）](#22-扩充规则由基本规则推导而来)
    - [3. 属性集闭包的计算（算法 6.1）](#3-属性集闭包的计算算法-61)
    - [4. Armstrong 公理的有效性与完备性（定理 6.2）](#4-armstrong-公理的有效性与完备性定理-62)
    - [5. 函数依赖集的覆盖与等价（定义 6.14）](#5-函数依赖集的覆盖与等价定义-614)
      - [5.1 覆盖](#51-覆盖)
      - [5.2 等价](#52-等价)
    - [6. 极小函数依赖集（定义 6.15）](#6-极小函数依赖集定义-615)
      - [6.1 定义（最小覆盖需满足 3 个条件）](#61-定义最小覆盖需满足-3-个条件)
      - [6.2 计算算法（定理 6.3）](#62-计算算法定理-63)
    - [7. 高频考点速记](#7-高频考点速记)
...
## Armstrong公理系统
- 概念：Armstrong 公理系统是一组用于从已知的函数依赖（Functional Dependency, FD）集中推导出更多函数依赖的推理规则。
### 1. Armstrong公理系统-基本规则
1. **A1 自反律**：若 $Y \subseteq X$，则 $X \rightarrow Y$（{学号，姓名} → 学号）
2. **A2 增广律**：若 $X \rightarrow Y$，则$XZ \rightarrow YZ$（学号→姓名 $\Rightarrow$ (学号，课程号) → (姓名，课程号)）
3. **A3 传递律**：若 $X\rightarrow Y$ 且 $Y\rightarrow Z$，则 $X\rightarrow Z$（学号→班级、班级→班主任 $\Rightarrow$ 学号→班主任）

### 2. Armstrong公理系统 -扩充规则
1. **合并规则**：若 X $\to$ Y 且 X $\to$ Z，则 X $\to$ YZ<br>学号 → 姓名、学号 → 班级 $\Rightarrow$ 学号 → (姓名, 班级)
2. **分解规则**：若 X $\to$ Y 且 Z $\subseteq$ Y，则 X $\to$ Z
<br>学号 → (姓名, 班级)$\Rightarrow$ 学号 → 姓名、学号 → 班级

3. **伪传递规则**：若 X $\to $Y 且 WY $\to$ Z，则 XW $\to$ Z
<br>学号 → 班级、(班级, 课程号) → 教室 $\Rightarrow$ (学号, 课程号) → 教室

- 引理：$X \to A_1A_2\cdots A_k \Rightarrow   X \to A_i( i=1,2,\dots,k)$ 成立
<br>学号 → (姓名, 班级, 专业) $\Rightarrow$ 学号 → 姓名，学号 → 班级，学号 → 专业

### 3. 核心基础概念

#### 3.1 函数依赖的逻辑蕴涵
- F 的所有规则，必然导致 X→Y 成立（X/Y是属性/属性集）。由F带来的，没有写出但必然成立的规则
<br>则称：F **逻辑蕴涵** X $\to$ Y，记作 F $\vDash$ X \to Y\

- ![alt text](image-61.png)
#### 3.2 函数依赖集的闭包 F^+（定义 6.12）
- 定义：$F^+ = { X \to Y \mid F \vDash X \to Y }$，即：**所有被 F 逻辑蕴涵的函数依赖的集合**。
...
​
#### 3.3 属性集的闭包 $X_F^+$（定义 6.13）
- 从 属性A 出发，用 F 中的依赖能推导出的**所有属性**的集合，$A_F^+$称为属性A关于函数依赖集F的闭包。

- 例：计算A关于F的闭包 $A_F^+$
  初始：$A_F^+ = {A}$ &emsp; 用 A→B：$A_F^+ = {A,B}$ &emsp; 用 B→C：$A_F^+ = {A,B,C}$
- 核心引理：$X→Y∈F^+ ⟺ Y⊆X_F^+$
- 计算方法：迭代算法
...
- 核心概念
    ...
    - **等价**：若 F覆盖G 且 G覆盖F，就说 F和G等价（充要关系）
- 例
  <br>$F = \{A \to B,\ B \to C\}$ 与 $G = \{A \to B,\ A \to C\}$

    设两个函数依赖集：F = \{A \to B,\ B \to C\}, \quad G = \{A \to B,\ A \to C\}
  

### 2. Armstrong 公理系统

#### 2.1 三条基本规则（公理）
设 R(U, F)，X,Y,Z \subse  teq U：
...
2.  **分解规则**
    ...
    - 推论：X \to A_1A_2\cdots A_k \iff \forall i, X \to A_i。
...
1.  初始化：X^{(0)} = X，i=0。
2.  迭代：
    - 找 F 中所有左部 \subseteq X^{(i)} 的依赖，将其右部属性集记为 Y。
    - 更新：X^{(i+1)} = X^{(i)} \cup Y。
3.  终止条件：X^{(i+1)} = X^{(i)} 或 X^{(i)} = U。此时 X^{(i)} 即为 X_F^+。

---

### 4. Armstrong 公理的有效性与完备性（定理 6.2）

1.  **有效性（Soundness）**
    - 由 F 经公理导出的任何函数依赖，一定在 F^+ 中。（不会推错）

2.  **完备性（Completeness）**
    - F^+ 中的所有函数依赖，都能经公理从 F 导出。（不会漏推）

- 结论：**“公理导出”与“逻辑蕴涵”完全等价**。

---

### 5. 函数依赖集的覆盖与等价（定义 6.14）

#### 5.1 覆盖
若 F 逻辑蕴涵 G 的所有依赖，则称 F **覆盖** G，记作 G \subseteq F^+。

#### 5.2 等价
若 F^+ = G^+，则称 F 与 G **等价**。

- 判定引理：F 与 G 等价 \iff F 覆盖 G 且 G 覆盖 F。

---

### 6. 极小函数依赖集（定义 6.15）

#### 6.1 定义（最小覆盖需满足 3 个条件）
设 F 为函数依赖集，F_m 是其极小依赖集，需满足：
1.  所有依赖的**右部均为单属性**。
2.  无冗余依赖：不存在 X \to A \in F_m，使得 F_m - \{X \to A\} 与 F_m 等价。
3.  左部无冗余属性：对任意 X \to A \in F_m，不存在 B \in X，使得 (F_m - \{X \to A\}) \cup \{(X-B) \to A\} 与 F_m 等价。

#### 6.2 计算算法（定理 6.3）
1.  **右部拆分**：将所有 X \to Y（|Y|>1）拆分为 X \to A（A \in Y）。
2.  **消除冗余依赖**：对每个 X \to A，若 A \in X_{F-\{X\to A\}}^+，则删除该依赖。
3.  **消除左部冗余属性**：对每个 X \to A，若存在 B \in X 且 A \in (X-B)_F^+，则用 (X-B) \to A 替代。
...
1.  **判断 X \to Y 是否成立**：计算 X_F^+，验证 Y \subseteq X_F^+。
2.  **求候选码**：找满足 X_F^+ = U 的**最小**属性集 X。
3.  **极小依赖集**：记住三步——拆右部、删冗余依赖、去左部冗余属性。